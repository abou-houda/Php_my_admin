...
class Table extends classeMereTable
{
    ...
    public function createInsertTable(
        $dbNom,
        $tableName,
        $fieldsName,
        $fieldsType,
    ...
    public function selectContraint($nomtable, $db)
    {
        $req = "select * from mytable where `nom`='$nomtable' and `db_nom`='$db'";
        $res = $this::selection($req)->fetchAll();
        if ($res[0][3] != "") {
            return explode("/", $res[0][3]);
        }
        return ["", ""];
    }
}
